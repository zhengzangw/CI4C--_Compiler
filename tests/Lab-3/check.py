...
for data_in, data_out, ret_val in json.load(open(f_json)):
    ...
    # Runtime error
    if ret != 0:
        err(data_in, data_out, "Runtime error occured when running your IR code")
        # Output mismatch
    with open(irsim_out, 'r') as from_irsim_r:
        from_irsim_r.readline()  # Filter out the first line "load ./workdir/a.ir"
        your_output = []
        try:
            temp_str = from_irsim_r.readline()
            while temp_str[:8] != "ret with":
